...
class Human(Animal): # ADDED HUMAN CLASS
    ...
    def __init__(self, island, x = 0, y = 0, s = "H"): # Defines init for humans. This uses the starve clock, hunt time and breed clocks 
    ...
    def clock_tick(self): # Reused code from the Predator Class, clock_tick
        ...
        '''
        self.breed_clock -= 1 # allows the clock to incriment down
        self.starve_clock -= 1# allows the clock to incriment down
...
###########################################
def main(predator_breed_time=6, predator_starve_time=3, initial_predators=15, prey_breed_time=5, initial_prey=52, \
         size=20, ticks=230, human_breed_time = 6, human_starve_time = 43, human_hunt_time = 13, inital_human = 18):
    ...
    # make an island
    isle = Island(size,initial_prey, initial_predators, inital_human) # ADDED HUMAN
